...
if (!isNumeric(nums)) {
    println("Podaj prawidłową wartość liczbową.")
    return
}

displayNumbersAsWords(nums)

def isNumeric(String text) {
    return text.toCharArray().every { it.isDigit() }
}

def displayNumbersAsWords(String numbersCollection) {
    def numbers = numbersCollection.collect { x -> Integer.parseInt(x) }
    numbers.eachWithIndex{ number, index ->
        displayNumberAsWord(number, index == 0)
    }
}

def displayNumberAsWord(int number, boolean isFirst) {
    def word = ""
